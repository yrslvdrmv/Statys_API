...
3. **System** call `Get product (partial search)` API method. In case of `Show active only checkbox` is activated **System** call `Get product (partial search status published)` API method
