...
3. **System** call `Get product (partial search)` API method. In case of 
4. **User** type one or several letters into search bar and press `Enter` button on keyboard `Show active only checkbox` is activated **System** call `Get product (full search status published)` A{PI method
